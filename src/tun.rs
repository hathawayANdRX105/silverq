...
use meow_common::{AdapterType, DnsMode, DelayHistory, Metadata, MeowError, MeowResult, Proxy, ProxyAdapter, ProxyConn, ProxyHealth, ProxyPacketConn, TunnelMode};
        ...
        if *current != new_tag {
            ...
            if let Some(tag) = &new_tag {
                if let Some(adapter) = registry.get(tag) {
                    ...
                    proxies.insert(smol_str::SmolStr::new("silverq-auto"), wrapped);
