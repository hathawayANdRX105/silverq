...
pub fn build_proxy(spec: &NodeSpec) -> Result<Arc<dyn ProxyAdapter>, String> {
    ...
    let proxy: Box<dyn ProxyAdapter> = match spec.protocol {
        ...
        crate::proxy::nodespec::Protocol::Trojan => {
        ...
        crate::proxy::nodespec::Protocol::Shadowsocks => {
            let s = spec
                .shadowsocks
                .as_ref()
...
pub fn build_transport(
    spec: &NodeSpec,
    v: &crate::proxy::nodespec::VlessSpec,
) -> Result<TransportChain, String> {
    use crate::proxy::nodespec::TransportSpec;
...
pub fn base64url_decode32(s: &str) -> Result<[u8; 32], String> {
    use base64::Engine;
    let s = s.trim();
...
pub fn hex_to_short_id(s: &str) -> Result<[u8; 8], String> {
    let hex = s.trim();
    if hex.len() > 16 {
